...
calculate_att_g_t = function(g_val, t_val, lookup_table, N_table, verbose = FALSE) {
    if (t_val >= g_val) {
        lag_t_val = g_val - 1
    } else {
        lag_t_val = t_val - 1
...
lookup_manual_fit[343, ]

estimate_did = function(data, y_var, group_var, t_var, id_var){
    data = df
    y_var = "first_Y"
    group_var = 'G' 
    t_var = "period"
    ...
    gs_and_ts_we_want[, att_g_t := att_estimates]
...
mb_results = microbenchmark::microbenchmark(
    ...
    rename(estimate = att_g_t, group = g, time = t),
    manual_binary_absorbing = map2_dfr(
        gs_and_ts$G,
        gs_and_ts$period,
